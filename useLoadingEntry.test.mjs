...
describe("React hook `useLoadingEntry`.", { concurrency: true }, () => {
  ...
  it("Without initial loading for each cache key used.", async () => {
  ...
  it("Initial loading for each cache key used.", async () => {
    const loading = new Loading();
    const cache = new Cache();
    const cacheKeyA = "a";
    const { promise: loadingA1Result, resolve: loadingA1ResultResolve } =
